# Infosystem — qué hacer al despertar (5–10 minutos)
...
**No**, si subes tú los archivos por Plesk o WinSCP. El agente no puede dejar la web perfecta en `centrosinfosystem.com` sin que alguien copie archivos al hosting.
...
2. **Borra** cualquier `infosystem-site-fixes.php`, `infosystem-global-ui-fix.php`, etc. (ver `tools/mu-plugins/_NO-SUBIR/README.md`)
3. Sube **solo** desde tu PC:
   - `eduma-child/tools/mu-plugins/infosystem-mu-pack.php`
...
2. En WordPress → Apariencia → Temas:
   - Activa **Eduma Child - Infosystem** (o el nombre del child)
...
Abre https://centrosinfosystem.com/
...
```php
define( 'INFOSYSTEM_CHILD_FULL', true );
```

Luego: Apariencia → **Infosystem Setup** (menú y páginas).
