<?php
/**
 * Landing «Eventos Infosystem» — agenda con personalidad (SEO CLM).
 *
 * @package eduma-child
 */
...
add_action(
	...
	static function () {
		...
		infosystem_ensure_eventos_page();
	},
	20
);
...
add_action(
	...
	static function () {
		...
		wp_enqueue_style(
			'infosystem-eventos',
			get_stylesheet_directory_uri() . '/assets/css/infosystem-eventos.css',
			array( 'eduma-child-infosystem' ),
			EDUMA_CHILD_VERSION
		);
		wp_enqueue_script(
			'infosystem-eventos',
			get_stylesheet_directory_uri() . '/assets/js/infosystem-eventos.js',
			array(),
			EDUMA_CHILD_VERSION,
			true
		);
