# Subir el proyecto con WinSCP (Infosystem)
...
1. **Apariencia → Temas** → Activar **Eduma Child - Infosystem** (versión `1.2.0`).
...
3. **Apariencia → Infosystem Setup** → **Ejecutar configuración** (solo si quieres recrear páginas/menú; la importación de cursos ya se ha hecho).
...
> Para forzar una nueva sincronización después de tocar el contenido en el código, sube el valor de `EDUMA_CHILD_COURSES_IMPORT_VERSION` en `inc/infosystem-import-courses.php` y vuelve a entrar al wp-admin.

También puedes ejecutar a mano la sincronización desde:
`/wp-admin/?eduma_child_sync_courses=1&_wpnonce=...` (botón en el asistente Infosystem Setup).
